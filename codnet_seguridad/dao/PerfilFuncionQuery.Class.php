...
class PerfilFuncionQuery {
	...
	function insertPerfilfuncion(Perfilfuncion $obj) {
		...
		$sql = "INSERT INTO ".  CDT_SEGURIDAD_TABLA_PERFIL_FUNCION . " ('cd_perfil' ,'cd_funcion') VALUES ('$cd_perfil' ,'$cd_funcion') ";
	...
	function modificarFuncionesDePerfil(Perfil $obj, Array $perfilFunciones) {
		...
		$cd_perfil = $obj->getCd_perfil ();
		$sql = "DELETE  FROM ".  CDT_SEGURIDAD_TABLA_PERFIL_FUNCION . " WHERE cd_perfil = $cd_perfil";
	...
	function tienePerfilFuncionesAsignadas (Perfilfuncion $obj) {
		$db = DbManager::getConnection();
		$cd_perfil = $obj->getCd_perfil();
		$sql = "Select count(*) as count FROM ".  CDT_SEGURIDAD_TABLA_PERFIL_FUNCION . " WHERE cd_perfil ='$cd_perfil'";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	function eliminarFuncionesDePerfil(Perfil $obj) {
		//me conecto a la BD
		$db = DbManager::getConnection();
		//Borro todas las filas de ese perfil
		$cd_perfil = $obj->getCd_perfil ();
