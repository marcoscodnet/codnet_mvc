 ...
 * 
 */
abstract class ReporteAction extends OutputAction{
	...
	protected function getXTemplate(){
		return new XTemplate(CDT_MVC_TEMPLATE_REPORTE);
	}
	...
	protected function parseHeader(XTemplate $xtpl){
		...
		if(!empty($pdf)){
			...
			$xtpl->parse( 'main.header.exportar.pdf');
		}

		$excel = $this->getAccionReporteExcel();
		if(!empty($excel)){
			$xtpl->assign ( 'accion_excel', $this->getAccionReporteExcel() );
			$xtpl->parse( 'main.header.exportar.excel');
		}
		
		$xtpl->parse( 'main.header.exportar');		
