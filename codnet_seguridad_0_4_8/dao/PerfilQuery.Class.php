...
class PerfilQuery {
	...
	static function insertarPerfil(Perfil $obj) {
		...
		$sql = "INSERT INTO ".  CDT_SEGURIDAD_TABLA_PERFIL . " (ds_perfil) VALUES ('$ds_perfil') ";
	...
	static function insert_id($db) {
		$db = DbManager::getConnection();
		$sql = "SELECT MAX(`cd_perfil`) as id FROM ".  CDT_SEGURIDAD_TABLA_PERFIL . " ";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	static function listar($cd_perfil = "") {
		$db = DbManager::getConnection();
		$sql = "SELECT cd_perfil, ds_perfil FROM ".  CDT_SEGURIDAD_TABLA_PERFIL . " ";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	static function getPerfilPorId(Perfil $obj) {
		...
		$sql = "SELECT * FROM ".  CDT_SEGURIDAD_TABLA_PERFIL . " WHERE cd_perfil = $cd_perfil";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	static function getCountPerfiles( CriterioBusqueda $criterio ) {
		$db = DbManager::getConnection();
		$sql = "SELECT count(*) as count FROM ".  CDT_SEGURIDAD_TABLA_PERFIL . " P";
	...
	static function modificarPerfil(Perfil $obj) {
		...
		$cd_perfil = $obj->getCd_perfil ();
		$ds_perfil = $obj->getDs_perfil ();
		$sql = "UPDATE ".  CDT_SEGURIDAD_TABLA_PERFIL . " SET ds_perfil='$ds_perfil'";
	...
	static function eliminarPerfil(Perfil $obj) {
		$db = DbManager::getConnection();
		$cd_perfil = $obj->getCd_perfil ();
		$sql = "DELETE FROM ".  CDT_SEGURIDAD_TABLA_PERFIL . " WHERE cd_perfil = $cd_perfil";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
