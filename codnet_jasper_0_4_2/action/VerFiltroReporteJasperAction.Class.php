 ...
 * 
 */
class VerFiltroReporteJasperAction extends OutputAction{
	...
	protected function parseFiltro( XTemplate $xtpl, FiltroReporteJasper $oFiltroReporteJasper ){
		...
		$label = utf8_decode( $reportUnit['label'] );
		$xtpl->assign ( 'REPORT_LABEL', $label );
		
		$description = utf8_decode( $reportUnit['description'] );
		...
		for ($i=0; $i< count($resources); ++$i){
			...
			if ($rd['type'] == TYPE_INPUT_CONTROL ){
				...
				$paramLabel = utf8_decode( $rd['label'] );
				...
				$paramDescription = utf8_decode( $rd['description'] );	
				$xtpl->assign ( 'PARAM_DESCRIPTION', $name );
