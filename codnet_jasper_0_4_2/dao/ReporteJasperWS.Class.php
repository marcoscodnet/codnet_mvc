...
class ReporteJasperWS{
	 ...
	 * @return unknown_type
	 */
	public static function getReportesJasper($uri = JASPER_URI){
	...
	public static function getReporteJasperUri( $uri, $cd_datasource="" ){
		...
		if (is_array($attachments)){
			...
			if ($_GET['format'] == RUN_OUTPUT_FORMAT_HTML){
				...
				$contenido =  "<div id='jasper' style='overflow:auto'>$contenido</div>";
				
				$volver = "<input type='button' onclick='javascript: history.back();'	value='Nueva b&uacute;squeda'>";
				$volver_listado = "<input type=\"button\" onclick=\"javascript: window.location='" . WEB_PATH.   "doAction?action=listar_reportesJasper&cd_datasource=$cd_datasource'\"	value=\"Volver al listado\">";
				
				$contenido .=  "<div style='text-align:center;' id='botones_jasper'> $volver &nbsp;&nbsp $volver_listado </div>";
