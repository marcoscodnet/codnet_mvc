...
class ActionMapHelper{
	...
	private $filter_map =  array(); // ( [nombre_filter] = clase_filter )
	...
	public function ActionMapHelper(){
		...
		foreach ($navegacion->getFilters() as $filter) {
			$this->setFilter($filter['nombre'], $filter);
		}
	}
	...
	function getForward($ds_key) {
		if( array_key_exists($ds_key, $this->forward_map ) )
			return $this->forward_map[$ds_key];
		else '';	
	}
	
	function getFilter($ds_key) {
		return $this->filter_map[$ds_key];
	}
	
	function getFilterClase($ds_key) {
		return $this->filter_map[$ds_key]['clase'];
	}

	function getFilterUrlPattern($ds_key) {
		return $this->filter_map[$ds_key]['urlPattern'];
	}
	...
	function setFilter($ds_filter_name,$filter){
		$this->filter_map[$ds_filter_name]=$filter;
	}
	...
	protected function initAccionesDefault($ds_entidad, $ds_entidad_plural=null){
		...
		//$this->setForward('alta_'.$ds_entidad.'_error','alta_'.$ds_entidad.'_init');
		//$this->setForward('alta_'.$ds_entidad.'_success','listar_'.$ds_entidad_plural);
		...
		//$this->setForward('modificar_'.$ds_entidad.'_error','modificar_'.$ds_entidad.'_init');
		//$this->setForward('modificar_'.$ds_entidad.'_success','listar_'.$ds_entidad_plural);
		
		//listar.
		$this->setAction('listar_'.$ds_entidad_plural, 'Listar'.$ds_entidad_plural_capital_letter.'Action');
		$this->setForward('listar_'.$ds_entidad_plural.'_error','doAction?action=listar_'.$ds_entidad_plural);
		//$this->setForward('listar_'.$ds_entidad_plural.'_error', 'listar_'.$ds_entidad_plural);
		...
		//$this->setForward('eliminar_'.$ds_entidad.'_error','listar_'.$ds_entidad_plural);
		//$this->setForward('eliminar_'.$ds_entidad.'_success','listar_'.$ds_entidad_plural);
