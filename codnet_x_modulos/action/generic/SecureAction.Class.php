...
abstract class SecureAction extends Action{
	 ...
	 * de usuario.
	 * (non-PHPdoc)
	 * @see clases/com/cdt/challenge/actions/Action#execute($funcion)
	 */
	public function execute(){
		...
		if (! $this->isUsuarioLogueado() ) {
			...
			$tienePermiso = PermisoQuery::permisosDeUsuario (  SeguridadUtils::getUsuarioLogueado()->getCd_usuario() , $this->getFuncion() );

			if (!$tienePermiso){
				//si no tiene permiso, forward a la página de acceso denegado.
				$forward = $this->acccesoDenegado();
			}else{
	...
	protected function acccesoDenegado(){
		return  'login_error';
	}

	protected function loginRequerido(){
		
		$ge = new GenericException("Debe loguearse en el sistema");
		$this->doForwardException( $ge, 'login_error');
	}

	protected function isUsuarioLogueado(){
		return SeguridadUtils::isUsuarioLogueado();
	}
	...
	public function tienePermisoLink($link){
		...
		*/
		$ds_action_value = $this->getAccionDeLink( $link );
