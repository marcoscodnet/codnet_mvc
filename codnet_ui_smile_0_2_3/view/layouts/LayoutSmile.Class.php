...
class LayoutSmile extends CdtLayout {
    ...
    protected function parseMenuGroup(XTemplate $xtpl, $menuGroup, $currentMenuGroup) {
        ...
        if ($menuGroup->hasPermission($oUser)) {
            ...
            $xtpl->assign('action_description', $menuGroup->getDs_name());
    ...
    protected function parseMenuOption(XTemplate $xtpl, $menuOption) {
        ...
        if ($menuOption->hasPermission($oUser)) {
            ...
            if (empty($description))
                $xtpl->assign('action_description', $menuOption->getDs_name());
            else
                $xtpl->assign('action_description', $description);
    ...
    protected function getHeader() {
        ...
        $xtpl->assign('app_titulo', CDT_MVC_APP_TITLE);
        $xtpl->assign('app_subtitulo', CDT_MVC_APP_SUBTITLE);
        $xtpl->parse('main');
        return $xtpl->text('main');
    }
    ...
    protected function parseStyles($xtpl) {
        ...
        $xtpl->assign('css', WEB_PATH . "css/smile/calendar.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/style_form.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/style_iefix.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/css_menu_panel.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/estilos.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/screensmall.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/jquery-ui-1.7.2.custom.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/jquery.alerts.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/jquery.ui.core.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/dock.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/jVal.css");
        $xtpl->parse('main.estilo');

        $xtpl->assign('css', WEB_PATH . "css/smile/cdt_secure/cdt_secure.css");
        $xtpl->parse('main.estilo');
    }
    ...
    protected function parseScripts($xtpl) {
        ...
        $xtpl->assign('js', WEB_PATH . "js/jquery/jquery-ui-timepicker-addon.js");
        $xtpl->parse('main.script');
        ...
        $xtpl->assign('js', WEB_PATH . "js/jquery/interface.js");
        $xtpl->parse('main.script');

        $xtpl->assign('js', WEB_PATH . "js/cdt_functions.js");
        $xtpl->parse('main.script');
    }
