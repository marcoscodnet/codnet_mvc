...
class Paginador {
	...
	private $num_rows_per_page; //Esta variable me indica la cantidad de registros que imprimo por pagina
	private $total_rows;
	...
	// Constructor
	//
	function Paginador($url, $numpages, $actualpage, $cssclassotherpage, $cssclassactualpage, $total_rows, $row_per_page=null) {
		...
		$this->num_rows_per_page = FormatUtils::ifEmpty($row_per_page, ROW_PER_PAGE);
	...
	//Debe estar incluído Ajax para poder utilizarlo
	function imprimirPaginadoConAjax($divId="") {
		$html = "";
		if ($this->getNumPages () > 1) {
			$html .= "<span class=\"tituloPaginador\">P&aacute;ginas:&nbsp;&nbsp;</span>";
			...
			if (($this->getActualPage ()) > 1) {
				$ds_pag_anterior = "&lt;&lt; anterior";
				$ant_page = ($this->getActualPage ()) - 1;
				$html .= "<a class=\"" . $this->getCssClassOtherPage () . "\" href=\"" . $this->getUrlAndGetVars ($ant_page) . "\" onClick=\"return invocarURLConAjax ('". $this->getUrlAndGetVars ($ant_page) . "', '".$divId."');\"  target=\"_self\">$ds_pag_anterior</a>&nbsp;&nbsp;";
			}
			
			if (($this->getInitPage ()) > 1) {
				$ds_pags_anteriores = "[.....]";
				$ant_pages = $this->getInitPage () - 1;
				$html .= "<a class=\"" . $this->getCssClassOtherPage () . "\" href=\"" . $this->getUrlAndGetVars ($ant_pages) . "\" onClick=\"return invocarURLConAjax ('". $this->getUrlAndGetVars ($ant_pages) . "', '".$divId."');\" target=\"_self\">$ds_pags_anteriores</a>&nbsp;&nbsp;";
			}
			
			for($i = $this->getInitPage (); $i <= ($this->getEndPage ()); $i ++) {
				if ($i != ($this->getActualPage ())) {
					$html .= "<a class=\"" . $this->getCssClassOtherPage () . "\" href=\"" . $this->getUrlAndGetVars ($i) . "\"  onClick=\"return invocarURLConAjax ('". $this->getUrlAndGetVars ($i) . "', '".$divId."');\" target=\"_self\">$i</a> ";
				} else {
					$html .= "<span class=\"" . $this->getCssClassActualPage () . "\">$i</span> ";
				}
			} //final del for
			...
			if ($this->getNumPages () > $this->getEndPage ()) {
				$ds_pags_siguientes = "[.....]";
				$sig_pages = $this->getEndPage () + 1;
				$html .= "&nbsp;<a class=\"" . $this->getCssClassOtherPage () . "\" href=\"" . $this->getUrlAndGetVars ($sig_pages) . "\" onClick=\"return invocarURLConAjax ('". $this->getUrlAndGetVars ($sig_pages) . "', '".$divId."');\" target=\"_self\">$ds_pags_siguientes</a>";
			}
			
			if (($this->getActualPage ()) < ($this->getNumPages ())) {
				$ds_pag_siguiente = "siguiente &gt;&gt;";
				$sig_page = ($this->getActualPage ()) + 1;
				$html .= "&nbsp;&nbsp;<a class=\"" . $this->getCssClassOtherPage () . "\" href=\"" . $this->getUrlAndGetVars ($sig_page) . "\" onClick=\"return invocarURLConAjax ('". $this->getUrlAndGetVars ($sig_page) . "', '".$divId."');\" target=\"_self\">$ds_pag_siguiente</a>";
			}
		}
		return $html;
	}
