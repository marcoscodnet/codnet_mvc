...
class CMPAutocomplete extends CMPComponent{
	...
	private $extraParam=""; //TODO ver para qué es.
	...
	//método a invocar sobre la clase anterior para obtener los items.
	private $requestMethod;

	//para determinar la clase del item.
	//si no se define nada el item será un array.
	private $itemClass;
	...
	public function setItemAttributesList( $value ){ $this->itemAttributesList = $value; }
	...
	public function getItems( $text ){
		...
		//si no se define el método requestMethod, usamos por default "getEntidades($criterio)" de la interface "IListar"
		$method = ($this->getRequestMethod())?$this->getRequestMethod():"getEntidades";
		
		//vemos si el parámetros es un criterio de búsqueda o sólo un texto.
		$oParameter = new ReflectionParameter(array($this->getRequestClass(), $method), 0);

		try{
		
			if( ($oParameter->getClass()!=null) && $oParameter->getClass()->getName() == 'CriterioBusqueda' ){
	...
	public  function formatItems( $items, $text ){
		...
		foreach ($items as $item) {
			...
			$content .= "\t".'<li class="' .  $li_style  .  '" id="autocomplete_'. $code .'" label="' .  $label .  '" rel="'. $rel .'">'. utf8_encode( $p ) .'</li>'."\n";
	...
	public function getCriterio($text){
		...
		$criterio->addFiltro( $this->itemLabel , $text, "like", new FormatValorLike());
		return $criterio;
	}
