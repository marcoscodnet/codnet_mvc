...
class FuncionQuery {
	...
	static function insertFuncion(Funcion $obj) {
		...
		$sql = "INSERT INTO ".  CDT_SEGURIDAD_TABLA_FUNCION . " ('cd_funcion' ,'ds_funcion') VALUES ('$cd_funcion' ,'$ds_funcion') ";
	...
	static function listarFunciones(Array $funciones) {
		$db = DbManager::getConnection();
		$sql = "SELECT cd_funcion, ds_funcion FROM ".  CDT_SEGURIDAD_TABLA_FUNCION . " ORDER BY ds_funcion";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	static function getDs_funcion(Funcion $obj) {
		$db = Db::conectar ();
		$cd_funcion = $obj->getCd_funcion ();
		$sql = "SELECT ds_funcion FROM ".  CDT_SEGURIDAD_TABLA_FUNCION . " WHERE cd_funcion = $cd_funcion";
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
	...
	static function getFuncionesDePerfil(Perfil $obj) {
		...
		$sql = "SELECT F.* FROM ".  CDT_SEGURIDAD_TABLA_PERFIL_FUNCION . " PF"; 
		$sql .= " LEFT JOIN ".  CDT_SEGURIDAD_TABLA_FUNCION . " F ON F.cd_funcion=PF.cd_funcion "; 
	...
	static function getFunciones() {
		$db = DbManager::getConnection();
		$sql = "SELECT * FROM ".  CDT_SEGURIDAD_TABLA_FUNCION . " order by cd_funcion "; 
		$result = $db->sql_query ( $sql );
		if(!$result)//hubo un error en la bbdd.
			throw new DBException();
		$funciones = ResultFactory::toCollection($db,$result,new FuncionFactory());	
	...
	static function getFuncionesDeUsuario(Usuario $obj) {
		...
		$sql = "SELECT F.* FROM ".  CDT_SEGURIDAD_TABLA_USUARIO . " U"; 
		$sql .= " LEFT JOIN ".  CDT_SEGURIDAD_TABLA_PERFIL_FUNCION . " PF ON PF.cd_perfil=U.cd_perfil "; 
		$sql .= " LEFT JOIN ".  CDT_SEGURIDAD_TABLA_FUNCION . " F ON F.cd_funcion=PF.cd_funcion ";
